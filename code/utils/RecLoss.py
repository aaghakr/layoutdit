...
def box_iou(boxes1, boxes2):
    ...
    iou = torch.where(union == 0, torch.zeros_like(inter), inter / union)
    return iou, union
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # C = C.clone()  # 避免修改原始tensor
        # C[torch.isnan(C)] = 1e9  # 将NaN替换为一个很大的有限值
        # C[torch.isinf(C)] = 1e9  # 将inf替换为一个很大的有限值
...
def generalized_box_iou(boxes1, boxes2):
    ...
    #
    # # 如果有一个边界框的面积为0,直接返回0
    # if (area1 == 0).any() or (area2 == 0).any():
    #     return torch.zeros_like(area1[:, None])
