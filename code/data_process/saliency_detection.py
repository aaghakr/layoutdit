...
def get_saliency_map(image):
    ...
    testers = [ISNetSaliencyTester(), BASNetSaliencyTester()]

    saliency_maps = []
    for tester in testers:
        saliency_map = tester(image)
        saliency_map = torch.squeeze(F.interpolate(saliency_map, (height, width), mode="bilinear"), 0)
        saliency_map = _norm_pred(saliency_map)
